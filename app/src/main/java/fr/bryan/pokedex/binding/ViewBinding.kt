...
package fr.bryan.pokedex.binding
...
import fr.bryan.pokedex.utils.PokemonTypeUtils
import fr.bryan.pokedex.utils.SpacesItemDecoration
...
object ViewBinding { //sert à binder des données avec les différente éléments graphiques
    ...
    types.whatIfNotNullOrEmpty {
      ...
      for (type in it) {
        with(recyclerView) {
          addRibbon(
            ribbonView(context) {
              setText(PokemonTypeUtils.getNameColor(type.type.name))
  ...
  @JvmStatic
  @BindingAdapter("bindPokemonAbilities")
  fun bindPokemonAbilities(recyclerView: RibbonRecyclerView, abilities: List<PokemonInfo.AbilityResponse>?) {
    abilities.whatIfNotNullOrEmpty {
      recyclerView.clear()
      for (ability in it) {
        with(recyclerView) {
          addRibbon(
            ribbonView(context) {
              setText(ability.ability.name)
              setTextColor(Color.WHITE)
              setPaddingLeft(84f)
              setPaddingRight(84f)
              setPaddingTop(2f)
